...
const reviewPlatforms = [
  {
    ...
    rating: '5.0',
    detail: '12 reviews',
    href: 'https://www.zillow.com/profile/Hoi%20Wing%20Ng',
  },
  {
    name: 'Yelp',
    rating: '5.0',
    detail: '34 reviews',
    href: 'https://www.yelp.com/biz/hoi-wing-ng-intero-real-estate-cupertino-2',
  },
  {
    name: 'Google',
    rating: '5.0',
    detail: 'Verified',
    href: 'https://www.google.com/search?q=Hoi+Wing+Ng+realtor+Cupertino+reviews',
  },
  {
    name: 'reach150',
    rating: '★',
    detail: 'Verified',
    href: 'https://www.reach150.com/Hoi-Wing-Ng',
  },
  {
    name: 'RealTrends',
    rating: 'Top 1%',
    detail: '$40M+ · 2024',
    href: 'https://www.realtrends.com',
  },
];
...
const neighborhoods = [
  { city: 'Cupertino',     county: 'Santa Clara County', median: '$2.7M', slug: 'cupertino',     tagline: 'Apple HQ, top-ranked schools, and the most sought-after zip codes in the valley.' },
  { city: 'Palo Alto',     county: 'Santa Clara County', median: '$3.8M', slug: 'palo-alto',     tagline: 'Stanford\'s quiet gravity, Eichler heritage, and concentrated Peninsula wealth.' },
  { city: 'Saratoga',      county: 'Santa Clara County', median: '$3.5M', slug: 'saratoga',      tagline: 'Estate living, award-winning schools, and privacy in the foothills.' },
  { city: 'Los Altos',     county: 'Santa Clara County', median: '$3.4M', slug: 'los-altos',     tagline: 'Village charm, walkable streets, and consistently strong appreciation.' },
  { city: 'Mountain View', county: 'Santa Clara County', median: '$2.0M', slug: 'mountain-view', tagline: 'Tech-forward community with excellent transit and lifestyle balance.' },
  { city: 'Sunnyvale',     county: 'Santa Clara County', median: '$2.1M', slug: 'sunnyvale',     tagline: 'High demand, diverse housing stock, and easy freeway access.' },
  { city: 'Menlo Park',    county: 'San Mateo County',   median: '$2.9M', slug: 'menlo-park',    tagline: 'Venture capital corridor, top schools, and a tight-knit community feel.' },
  { city: 'Atherton',      county: 'San Mateo County',   median: '$7.5M', slug: 'atherton',      tagline: 'One of the most expensive zip codes in the US — private, prestigious, understated.' },
];
    ...
    <div className="space-y-24 lg:space-y-32">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative h-[420px] w-full lg:hidden">
          <Image src="/hoi-headshot.png" alt="Hoi Wing Ng" fill className="object-cover object-[50%_8%]" priority />
      ...
      <section className="mx-auto max-w-7xl px-6 sm:px-10">
        <div className="mb-10">
          <p className="text-[11px] uppercase tracking-[0.35em] text-brand-accent/80">Verified reviews</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
            Rated 5 stars across every platform
          </h2>
        </div>
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {reviewPlatforms.map(({ name, rating, detail, href }) => (
            <a
              key={name}
              href={href}
              target="_blank"
              rel="noreferrer"
              className="group flex flex-col items-center rounded-2xl border border-white/[0.07] bg-white/[0.02] px-5 py-6 text-center transition-all duration-300 hover:border-brand-accent/30 hover:bg-white/[0.05]"
            >
              <p className="text-[22px] font-semibold text-brand-accent transition-colors duration-200 group-hover:text-brand-accent">
                {rating}
              </p>
              <p className="mt-1 text-[13px] font-medium text-white">{name}</p>
              <p className="mt-0.5 text-[11px] text-white/40">{detail}</p>
              <p className="mt-3 text-[11px] text-brand-accent/50 transition-all duration-200 group-hover:text-brand-accent">
                View reviews ↗
              </p>
            </a>
          ))}
      ...
      <section className="mx-auto max-w-7xl px-6 sm:px-10">
        <div className="mb-14 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Expert guidance at every stage</h2>
          </div>
          <p className="max-w-xs text-[15px] leading-7 text-white/40">Every plan focuses on durable value — not a rushed transaction.</p>
        </div>
        <div className="grid gap-px overflow-hidden rounded-2xl bg-white/[0.06] lg:grid-cols-3">
          {services.map(({ num, title, description, href }) => (
            <Link key={href} href={href} className="group relative flex flex-col overflow-hidden bg-[#0e1113] p-9 transition-all duration-300 hover:bg-white/[0.04]">
              <span className="pointer-events-none absolute -right-2 -top-4 select-none text-[96px] font-bold leading-none text-white/[0.03] transition-all duration-500 group-hover:text-white/[0.07]">{num}</span>
              <p className="mb-4 text-[10px] uppercase tracking-[0.3em] text-brand-accent/50 transition-colors duration-300 group-hover:text-brand-accent/80">{num}</p>
              <h3 className="text-[16px] font-semibold text-white/90 transition-colors duration-300 group-hover:text-brand-accent">{title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-6 sm:px-10">
        <div className="mb-12">
          <p className="text-[11px] uppercase tracking-[0.35em] text-brand-accent/80">Where we work</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Silicon Valley communities</h2>
          <p className="mt-4 max-w-xl text-[16px] leading-7 text-white/45">
            Deep market knowledge across Santa Clara and San Mateo counties — from starter homes to estate properties.
          </p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {neighborhoods.map(({ city, county, median, slug, tagline }) => (
            <Link
              key={slug}
              href={`/communities/${slug}`}
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 transition-all duration-300 hover:border-brand-accent/25 hover:bg-white/[0.05]"
            >
              <p className="text-[10px] uppercase tracking-[0.25em] text-white/30">{county}</p>
              <h3 className="mt-2 text-[17px] font-semibold text-white transition-colors duration-200 group-hover:text-brand-accent">
                {city}
              </h3>
              <p className="mt-2 flex-1 text-[13px] leading-6 text-white/45">{tagline}</p>
              <div className="mt-5 flex items-center justify-between border-t border-white/[0.07] pt-4">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.2em] text-white/30">Median</p>
                  <p className="text-[15px] font-semibold text-brand-accent">{median}</p>
                </div>
                <span className="text-[12px] text-brand-accent/40 transition-all duration-200 group-hover:translate-x-1 group-hover:text-brand-accent">
                  View listings →
                </span>
              </div>
            </Link>
      ...
      <section className="mx-auto max-w-7xl px-6 sm:px-10">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {testimonials.map(({ quote, author, source, href }) => (
            <figure key={source} className="group flex flex-col rounded-2xl border border-white/[0.06] bg-white/[0.02] p-8 transition-all duration-300 hover:border-brand-accent/25 hover:bg-white/[0.04]">
              ...
              <figcaption className="mt-7 flex items-center justify-between border-t border-white/[0.07] pt-5">
                <div>
                  <p className="text-[13px] font-semibold text-white">{author}</p>
                  <p className="mt-0.5 text-[12px] text-white/35">{source} · Verified</p>
                </div>
                <a
                  href={href}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg border border-white/10 px-3 py-1.5 text-[11px] text-white/40 transition-all duration-200 hover:border-brand-accent/40 hover:text-brand-accent"
                >
                  Read ↗
                </a>
              </figcaption>
        ...
        {/* All review platform links — prominent, not tiny text */}
        <div className="mt-8 flex flex-wrap gap-3">
          {reviewPlatforms.map(({ name, href, detail }) => (
            <a
              key={name}
              href={href}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-full border border-white/[0.1] bg-white/[0.03] px-4 py-2 text-[13px] text-white/50 transition-all duration-200 hover:border-brand-accent/40 hover:bg-brand-accent/5 hover:text-brand-accent"
            >
